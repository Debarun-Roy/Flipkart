...
#selenium driver implementation on heroku
...
class ThreadClass:
    ...
    def __init__(self, expected_review, searchString, scrapper_object, review_count):
        ...
        thread.daemon = True #daemonize started
        thread.start() #start execution
    
    def run(self):
        global collection_name, free_status
        free_status = False
        collection_name = self.scrapper_object.getReviewsToDisplay(expected_review=self.expected_review,searchString=self.searchString,username='Kavita',password='kavita1610',review_count=self.review_count)
...
def index():
    if request.method == 'POST':
        ...
        if free_status != True:
            ...
            free_status=True
            searchString = request.form['content'].replace(" ","")  #obtaining the searched string entered in the form
            expected_review = int(request.form['expected_review'])
            try:
                review_count=0
                scrapper_object = FlipkartScrapper(executable_path=ChromeDriverManager().install(), chrome_options=chrome_options)
...
def feedback():
    try:
        ...
        if collection_name is not None:
            scrapper_object = FlipkartScrapper(executable_path=ChromeDriverManager().install(),chrome_options=chrome_options)
            mongoClient = MongoDBManagement(username='Kavita', password='kavita1610')
            rows = mongoClient.findAllRecords(dbname="Flipkart-Scrapper", collection_name=collection_name)
